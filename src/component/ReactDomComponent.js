...
import instantiateReactComponent, {
  shouldUpdateReactComponent,
  arrayToMap, insertChildAt,
  deleteChild,
  Event
} from './util'
...
export default class extends ReactComponent {
  ...
  mountComponent(rootId) {
    ...
    // 设置tag，加上标识
    let tagOpen = `${type} data-reactcoreid=${this._rootNodeId}`,
      tagClose = `/${type}`,
      content = ''
  ...
  _updateDOMProperties(lastProps, nextProps) {
    ...
    for (propKey in lastProps) {
      ...
      if (!nextProps.hasOwnProperty(propKey)) {
        document.querySelector(`[data-reactcoreid="${this._rootNodeId}"]`).removeAttribute(propKey)
      }
    }
    ...
    for (let name in nextChildComponents) {
      ...
      if (prevChildComponent === nextChildComponent) {
        ...
        prevChildComponent._mountIndex < lastIndex && diffQueue.push({
          ...
          parentNode: document.querySelector(`[data-reactcoreid="${this._rootNodeId}"]`),
    ...
    updates.forEach(({ type, fromIndex, toIndex, parentNode, parentId, markup, id }) => {
      ...
      switch (type) {
        ...
        case UPDATE_TYPES.MOVE_EXISTING:
          deleteChild(parentNode, document.querySelector(`[data-reactcoreid="${id}"]`))
